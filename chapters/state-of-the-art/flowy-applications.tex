...
\end{center}
\end{figure}
The authors also looked into the geographical affiliations of different users by analyzing the \ac{ccTLD} of the browsed websites. They proposed a hypothesis that a user's origins strongly \marginpar{geographical preferences} influences its browsing activity in relation to the destination (or language) oriented websites it usually visits. The analysis of the results established that the top five visited \ac{ccTLD}s constituted more than $85\%$ of the overall number of a user's visits (excluding generic TLDs) as shown in figure \ref{fig:cybermetrics-geography}.


The user feature extraction and eventual evaluation was performed on two different data sets. One of the data sets contained flow-records from known number of users and the other at a metering point of a university containing potentially a number of unknown users. The evaluation results testified the influence of the extracted features in potential user differentiation. 
