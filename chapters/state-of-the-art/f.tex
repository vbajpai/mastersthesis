%**************************************
\chapter{Flowy $\longrightarrow$ F$(v1)$}\label{ch:f}
%**************************************
...
levels of inheritance. The \texttt{flowquery} \marginpar{flowquery and
branchinfo struct} struct for instance, is the parent of all the rule
...
The group-filter struct is similar to the filter struct previously shown in
listing $\ref{lst:filterrule}$. The grouper struct is shown in listing
$\ref{lst:grouperrule}$ and is used to perform relative comparison on the
flow-records. It takes in offsets of the fields to be grouped, their lengths
and a comparison function. Possible \marginpar{grouper and group struct}
comparison functions are \texttt{eq}, \texttt{ne}, \texttt{lt}, \texttt{gt},
\texttt{le} and \texttt{ge}. The comparison function creates a \texttt{group}
instance, a pointer to which is passed to it. The group struct is shown in
listing $\ref{lst:grouprule}$ which apart from the information about the
members, also points to a grouper aggregation struct that contains
meta-information resulting from calling an aggregation function.
...
aggregation function. \marginpar{grouper aggregation struct} Possible
...
comparison operations. \marginpar{rules in cnf} The comparison (\texttt{>>}
...
\marginpar{efficient rule processing using function pointers and switch
cases} using a function name derived from the combination of field length,
...
\cite{phaag:2006}. Since these \marginpar{f vs \{flow-tools, nfdump\}} tools
